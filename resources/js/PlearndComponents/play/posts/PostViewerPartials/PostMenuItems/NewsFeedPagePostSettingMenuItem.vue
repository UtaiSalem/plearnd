...
const handleDeletePost = async () => {
    ...
    // }
    // const deletePostResp = await axios.delete('/posts/'+activity.target_resource.id);
                ...
                <MenuItems
                    ...
                    <div class="py-1">
                        ...
                        <MenuItem v-slot="{ active }" v-if="postSettingMenus[1].isAuthor">
                            <Link :href="postSettingMenus[1].url"
                        ...
                        <!-- <MenuItem v-slot="{ active }" v-if="!postSettingMenus[3].isAuthor">
                            <Link :href="activity.target_resource.post_url"
                                :class="[active ? 'bg-gray-100 ' : postSettingMenus[3].color]"
                                class="flex items-start justify-start gap-2 p-2 px-5 transition-colors duration-300 hover:bg-emerald-50 focus:bg-emerald-50 focus:text-emerald-600 focus:outline-none focus-visible:outline-none">
                                <Icon :icon="postSettingMenus[3].icon" class="w-5 h-5" :class="'text-' + postSettingMenus[3].color" />
                                <span class="flex flex-col gap-1 overflow-hidden whitespace-nowrap">
                                    <span class="leading-5 truncate" :class="'text-'+postSettingMenus[3].color">{{ postSettingMenus[3].name }}</span>
                                </span>
                            </Link>
                        </MenuItem> -->
                    </div>
