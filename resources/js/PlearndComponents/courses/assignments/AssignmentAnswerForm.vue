...
const newAsmAnsForm = reactive({
    ...
    content: 'ส่งคำตอบ'+ usePage().props.auth.user.name
});
...
<template>
    <div class="m-4">
        <form @submit.prevent="handlePostCommentFormSubmit" :id="`comment-form-${assignment_id}`" class="mt-3 relative dark:border-secondary-800 flex items-start">
            ...
            <div class="relative w-full ml-2">
                ...
                <small class="absolute flex justify-between w-full px-2 py-1 text-xs transition text-slate-400 peer-invalid:text-pink-500">
                    <span></span>
                    <span class="text-slate-500"> {{ newAsmAnsForm.content.length }} </span>
                </small>
            ...
            <button @click.prevent="onSubmitAsmAns"
                ...
                :class="canSubmit ? 'hover:scale-110 text-violet-500 border-violet-500': 'cursor-not-allowed text-gray-500 border-gray-500 '"
