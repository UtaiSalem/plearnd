...
async function handleConfirmAnswer() {
    ...
    let resultResp = await axios.put(`/users/${usePage().props.auth.user.id}/answers/${selectedOption.value}/questions/${props.question.id}`);
    if (resultResp.data.success) {
        // console.log(resultResp.data);
        Swal.fire('สำเร็จ','ส่งคำตอบเสร็จเรียบร้อยแล้ว.','success' );
            ...
            <div id="hs-basic-with-title-and-arrow-stretched-collapse-one"
                ...
                <div class="ml-4 mt-4">
                    ...
                    <div class="my-2 flex items-center justify-center" v-if="!props.isCourseOwner">
                        ...
                        <button type="button" :id="`btn-confirm-answer${props.question.id}`" v-if="showConfirmAnswerButton || props.question.options.length " @click.prevent="handleConfirmAnswer()" :disabled="!selectedOption"
                        ...
                        <button type="button" :id="`btn-confirm-answer${props.question.id}`" v-if="showEditAnswerButton || props.question.options.length " @click.prevent="setCanEditAnswer"
