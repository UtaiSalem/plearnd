...
<template>
    <div class="" >
        <div class="bg-white border-t-4 border-blue-600 rounded-lg overflow-hidden shadow-lg my-4">
            <div class="m-4 text-xl font-semibold">
                หัวข้อของบทเรียน (  {{ props.topics.length }} ) หัวข้อ
                <p v-if="props.topics.length<1" class="text-sm font-thin"> 
                    (ยังไม่มีหัวข้อของบทเรียน)
                </p>
            </div>
        </div>
            ...
            <div v-for="(topic,index) in props.topics" :key="topic.id"  class="tabs flex flex-col justify-center mt-4 rounded-lg bg-white shadow-lg text-gray-700 overflow-hidden">
                <div class="relative">
                    <div class="absolute top-1 right-4" v-if="$page.props.isCourseAdmin">
                        ...
                        <DotsDropdownMenu 
                            :index="index" 
                            @delete-model="handleDeleteTopic(index, topic.id)"
                        >
