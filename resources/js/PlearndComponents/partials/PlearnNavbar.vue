...
<template>
    <header class="fixed top-0 shadow-md p-2 mx-auto w-full flex bg-[#34465d] justify-between z-30">
      <!-- logo && Title -->
      <div class="flex justify-between items-center space-x-3 p-2">
        <img src="https://vasterra.com/blog/wp-content/uploads/2021/08/Tailwind-img.png" alt="" class="h-10" />
        <Link :href="route('dashboard')">
        <span class="hidden sm:block text-2xl font-semibold text-white uppercase">Plearn</span>
        </Link>
      </div>
  
      <div class="lg:flex items-center justify-center hidden w-1/3">
        <form class="w-full">
          <div class="flex">
            <div class="relative w-full">
              <input type="search" id="search-dropdown"
                class="block p-2.5 w-full z-20 text-sm text-gray-900 bg-gray-50 rounded-lg border-l-gray-50 border-l-2 border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-l-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:border-blue-500 focus:outline-none"
                placeholder="Search..." required />
              <button type="submit"
                class="absolute top-0 right-0 p-2.5 text-sm font-medium text-white bg-blue-700 rounded-r-lg border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                <Icon icon="heroicons:magnifying-glass-circle-solid" class="w-5 h-5" />
      ...
      <div class="flex items-center justify-between space-x-3">
        ...
        <button type="button"
          class="inline-flex relative items-center p-2 text-sm font-medium text-center text-white hover:text-gray-800 rounded-full hover:bg-gray-200 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
          ...
          <div
            class="inline-flex absolute -top-2 -right-2 justify-center items-center w-6 h-5 text-xs font-bold text-white bg-purple-500 rounded-full dark:border-gray-900">
            8
          </div>
        </button>
        ...
        <button type="button"
          ...
          <div
            class="inline-flex absolute -top-2 -right-2 justify-center items-center w-6 h-5 text-xs font-bold text-white bg-red-500 rounded-lg dark:border-gray-900">
            6
          </div>
        </button>
      ...
      <div class="flex items-center text-white space-x-1">
        <img :src="$page.props.profilePath + 'storage/images/pln-coin.png'" alt="" class="w-8 h-8 hidden sm:block" />
        <div class="flex">
          <span class="sm:hidden text-yellow-300">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
              <path fill-rule="evenodd"
          ...
          <span>{{ $page.props.auth.user.personal_point }}</span>
        </div>
      </div>
  
      <div class="flex items-center space-x-3">
        <div class="hidden md:block font-semibold dark:text-white text-white">
          <div>{{ $page.props.auth.user.name }}</div>
        </div>
        <div class="avatar xonline">
          <div class="w-10 rounded-full ring ring-info-100 ring-offset-base-100 ring-offset-2">
            <img :src="$page.props.profilePath + $page.props.auth.user.avatar" />
          </div>
          <span
            class="bottom-0 left-7 absolute w-2.5 h-2.5 bg-green-400 border-2 border-white dark:border-gray-800 rounded-full"></span>
        </div>
  
        <div class="flex items-center justify-center text-white">
          <!-- Dropdown toggle button -->
          <button @click.prevent="toggleSettingMenu" @focusout="isOpen = false"
            class="relative z-10 block p-2 border border-transparent rounded-full text-white focus:border-blue-500 focus:ring-opacity-40 dark:focus:ring-opacity-40 focus:ring-blue-300 dark:focus:ring-blue-400 focus:ring dark:bg-gray-800 focus:outline-none">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
              <path fill-rule="evenodd"
          ...
          <!-- Dropdown menu -->
          <div v-if="isOpen"
            class="absolute right-10 top-14 z-20 w-56 py-2 mt-2 overflow-hidden bg-white rounded-md shadow-xl dark:bg-gray-800">
            <a href="#"
