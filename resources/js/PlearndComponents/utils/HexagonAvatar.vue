...
const badgeTextStyle = computed(() => {
  ...
  return {
    position: 'absolute',
    color: '#ffffff',
    fontSize: props.responsive ? `${Math.max(7, size * 0.35)}px` : '14px', // ลดขนาดตัวอักษร
    fontWeight: 'bold',
...
function createHexagonPath(ctx) {
  ...
  // ใช้ชั้นเดียวสำหรับทุกระดับการศึกษา
  const actualBorderLayers = 1;
  
  // คำนวณรัศมีโดยรวมขอบสีขาว 8px และขอบชั้นเดียว
  const outerBorderWidth = 8;
  const totalBorderWidth = outerBorderWidth + (props.lineWidth * actualBorderLayers);
  const radiusX = width / 2 - totalBorderWidth / 2;
  const radiusY = height / 2 - totalBorderWidth / 2;
...
function drawHexagonBorders(ctx) {
  ...
  // เพิ่มขอบสีขาว 4px ที่ขอบนอกสุด
  ...
  ctx.beginPath();
  const outerRadiusX = width / 2 - outerBorderWidth / 2;
  const outerRadiusY = height / 2 - outerBorderWidth / 2;
  
  if (props.roundedCorners && props.roundedCornerRadius > 0) {
    drawPerfectRoundedHexagon(ctx, width / 2, height / 2, outerRadiusX, outerRadiusY, props.roundedCornerRadius);
  } else {
    drawPerfectHexagon(ctx, width / 2, height / 2, outerRadiusX, outerRadiusY);
  }
  
  ctx.closePath();
  ctx.stroke();
  
  // วาดเส้นขอบชั้นเดียว - แสดงปีการศึกษาด้วยสี
  const layerOffset = 0;
  const adjustedLineWidth = lineWidth;
  
  const centerX = width / 2;
  const centerY = height / 2;
  const radiusX = width / 2 - outerBorderWidth - layerOffset - adjustedLineWidth / 2;
  const radiusY = height / 2 - outerBorderWidth - layerOffset - adjustedLineWidth / 2;
  
  // ใช้สีตามปีการศึกษา (gradeLevel)
  const layerColor = gradeLevelColors.value;
  
  // วาดเส้นขอบแบบมีด้านสีขาว - แสดงความคืบหน้าปีการศึกษา
  drawPartialHexagonBorder(ctx, centerX, centerY, radiusX, radiusY, layerColor, props.gradeLevel);
  
  // เพิ่มขอบสีขาวด้านใน 4px
  ...
  ctx.beginPath();
  const innerRadiusX = width / 2 - outerBorderWidth - adjustedLineWidth - innerBorderWidth / 2;
  const innerRadiusY = height / 2 - outerBorderWidth - adjustedLineWidth - innerBorderWidth / 2;
  
  if (props.roundedCorners && props.roundedCornerRadius > 0) {
    drawPerfectRoundedHexagon(ctx, centerX, centerY, innerRadiusX, innerRadiusY, props.roundedCornerRadius);
  } else {
    drawPerfectHexagon(ctx, centerX, centerY, innerRadiusX, innerRadiusY);
  }
