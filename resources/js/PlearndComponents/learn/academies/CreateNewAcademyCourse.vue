...
// const courseEndDate = new Date(new Date().setDate(courseStartDate.getDate() + 120));
...
function handleDateSelect(modelData){
  ...
  // form.value.start_date = courseRange.value[0];
  // form.value.end_date = courseRange.value[1];
  form.value.start_date = new Date(modelData[0]).toDateString();
  form.value.end_date = new Date(modelData[1]).toDateString();
}
...
async function handleSubmitForm(){
  try {
    ...
    courseFormData.append('academy_id', form.value.academy_id);
    ...
    courseFormData.append('start_date', form.value.start_date);
    courseFormData.append('end_date', form.value.end_date);
    courseFormData.append('auto_accept_members', form.value.auto_accept_members);
    courseFormData.append('tuition_fees', form.value.tuition_fees);
    courseFormData.append('saleable', form.value.saleable);
    courseFormData.append('price', form.value.price);
    courseFormData.append('status', form.value.status);
    
    // courseFormData.append('cover', tempCover.value === '/storage/images/courses/covers/default_cover.jpg' ? null : form.value.cover);
    form.value.cover ? courseFormData.append('cover', form.value.cover): null;

    const courseResp = await axios.post(`/academies/${props.academy.data.id}/courses`, courseFormData , config);
    // const courseResp = await axios.post(`/academies/${props.academy.data.id}/courses`, form.value , config);
...
<template>
  <div class="w-full md:mx-4 bg-white rounded-xl shadow-xl py-2">
    ...
    <div class="m-4">
      <form @submit.prevent="handleSubmitForm" class="mt-2 space-y-2" id="create-new-course-form">
        <div class="bg-white rounded-t-lg shadow-xl mb-6">
        ...
        <div class="max-w-sm space-y-2 sm:flex items-center sm:space-y-0 sm:space-x-2">
            <img class="block mx-auto h-14 rounded-full sm:mx-0 sm:shrink-0" :src="$page.props.auth.user.profile_photo_url" :alt="$page.props.auth.user.name">
        ...
        <div class="grid grid-cols-1 gap-2 my-2">
          ...
          <div class="flex justify-between items-center relative">
            ...
            <button type="button" @click.prevent="()=>{ isOpenCategoryOptions = !isOpenCategoryOptions }" class="bg-blue-600 w-8 rounded-r-lg h-full flex justify-center items-center text-white">
        ...
        <div class="grid grid-cols-1 gap-2 my-2">
          ...
          <div class="flex justify-between items-center relative">
            ...
            <button type="button" @click.prevent="()=>{ isOpenLevelOptions = !isOpenLevelOptions }" class="bg-blue-600 w-8 rounded-r-lg h-full flex justify-center items-center text-white">
                <Icon icon="heroicons-solid:chevron-down" class="w-9 h-9" />
            </button>
        ...
        <div class="grid sm:grid-cols-2 gap-2">
          ...
          <div class="relative sm:my-3">
              ...
              <label for="course-credit_unit-input" class="cursor-text peer-focus:cursor-default peer-autofill:-top-2 absolute left-2 -top-2 z-[1] px-2 text-xs text-slate-400 transition-all before:absolute before:top-0 before:left-0 before:z-[-1] before:block before:h-full before:w-full before:bg-white before:transition-all peer-placeholder-shown:top-4 peer-placeholder-shown:left-12 peer-placeholder-shown:text-sm peer-required:after:text-pink-500 peer-required:after:content-['\00a0*'] peer-invalid:text-pink-500 peer-focus:-top-2 peer-focus:left-2 peer-focus:text-xs peer-focus:text-violet-500 peer-invalid:peer-focus:text-pink-500 peer-disabled:cursor-not-allowed peer-disabled:text-slate-400 peer-disabled:before:bg-transparent">
                  หน่วยกิต
              </label>
              <Icon icon="fluent:number-symbol-square-24-regular" class="absolute w-7 h-7 top-3 left-4 text-violet-600 peer-disabled:cursor-not-allowed" />
          </div>

          <div class="relative sm:my-3">
              <input id="course-hpw-input" type="number" min="0" v-model="form.hours_per_week" placeholder="" class="relative w-full h-12 px-4 pl-12 placeholder-transparent transition-all border rounded-lg outline-none focus-visible:outline-none peer border-slate-300 text-slate-500 autofill:bg-white invalid:border-pink-500 invalid:text-pink-500 focus:border-violet-500 focus:outline-none invalid:focus:border-pink-500 disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400" />
              <label for="course-hpw-input" class="cursor-text peer-focus:cursor-default peer-autofill:-top-2 absolute left-4 -top-2 z-[1] px-2 text-xs text-slate-400 transition-all before:absolute before:top-0 before:left-0 before:z-[-1] before:block before:h-full before:w-full before:bg-white before:transition-all peer-placeholder-shown:top-4 peer-placeholder-shown:left-10 peer-placeholder-shown:text-sm peer-required:after:text-pink-500 peer-required:after:content-['\00a0*'] peer-invalid:text-pink-500 peer-focus:-top-2 peer-focus:left-2 peer-focus:text-xs peer-focus:text-violet-500 peer-invalid:peer-focus:text-pink-500 peer-disabled:cursor-not-allowed peer-disabled:text-slate-400 peer-disabled:before:bg-transparent">
                  คาบ / สัปดาห์
              </label>
              <Icon icon="bi:calendar4-week" class="absolute w-5 h-5 top-4 left-4 text-violet-600 peer-disabled:cursor-not-allowed" />
          </div>
        ...
        <div class="grid grid-cols-1 gap-4 my-2">
          <div class="relative flex flex-wrap items-center mt-2 mb-2">
              <input 
                type="checkbox" id="autoAcceptCourseMemberInput" v-model="form.auto_accept_members" :checked="form.auto_accept_members" 
              ...
              />
              <label class="pl-2 cursor-pointer text-slate-500 peer-disabled:cursor-not-allowed peer-disabled:text-slate-400" for="autoAcceptCourseMemberInput">
        ...
        <div class="grid grid-cols-1 gap-2 mb-1">
          ...
          <div class="relative mb-4">
              <input id="course-fees-input" type="number" v-model="form.tuition_fees" placeholder="" class="relative w-full h-12 px-4 pl-12 placeholder-transparent transition-all border rounded outline-none focus-visible:outline-none peer border-slate-300 text-slate-500 autofill:bg-white invalid:border-pink-500 invalid:text-pink-500 focus:border-violet-500 focus:outline-none invalid:focus:border-pink-500 disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400" />
              <label for="course-fees-input" class="cursor-text peer-focus:cursor-default peer-autofill:-top-2 absolute left-2 -top-2 z-[1] px-2 text-xs text-slate-400 transition-all before:absolute before:top-0 before:left-0 before:z-[-1] before:block before:h-full before:w-full before:bg-white before:transition-all peer-placeholder-shown:top-3 peer-placeholder-shown:left-10 peer-placeholder-shown:text-sm peer-required:after:text-pink-500 peer-required:after:content-['\00a0*'] peer-invalid:text-pink-500 peer-focus:-top-2 peer-focus:left-2 peer-focus:text-xs peer-focus:text-violet-500 peer-invalid:peer-focus:text-pink-500 peer-disabled:cursor-not-allowed peer-disabled:text-slate-400 peer-disabled:before:bg-transparent">
              ...
              <small class="absolute flex justify-between w-full mb-4 py-1 text-xs transition text-red-400 peer-invalid:text-pink-500">
        ...
        <div class="relative flex items-center mt-6 mb-2">
          ...
          <label class="pl-2 mt-6 cursor-pointer text-slate-500 peer-disabled:cursor-not-allowed peer-disabled:text-slate-400"
          ...
          <p class="ml-2 mt-6">บาท</p>
        </div>

        <div class="flex space-x-2">
          <p class="text-sm">ปิด</p>
          <input type="checkbox" v-model="form.status" class="peer sr-only opacity-0" id="input-course-status-toggle" />
          <label for="input-course-status-toggle" class="relative flex h-6 w-11 cursor-pointer items-center rounded-full bg-gray-400 px-0.5 outline-gray-400 transition-colors before:h-5 before:w-5 before:rounded-full before:bg-white before:shadow before:transition-transform before:duration-300 peer-checked:bg-green-500 peer-checked:before:translate-x-full peer-focus-visible:outline peer-focus-visible:outline-offset-2 peer-focus-visible:outline-gray-400 peer-checked:peer-focus-visible:outline-green-500">
        ...
        <div class="grid grid-cols-1 gap-4">
          <div class="flex items-center justify-around mt-4">
            <button type="button" 
              class="px-6 py-2 text-white bg-red-400 hover:bg-red-500 hover:text-white rounded"
            >
              ยกเลิก
            </button>
            ...
            <button
              type="submit"
              class="px-6 py-2 text-gray-700 border bg-blue-500 hover:bg-blue-500 rounded-lg"
            >
              บันทึก
            </button>
          </div>
