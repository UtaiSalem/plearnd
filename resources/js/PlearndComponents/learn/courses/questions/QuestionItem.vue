...
async function handleConfirmAnswer() {
    try {
        ...
        //Update user answer
            ...
            if (usePage().props.auth.user.pp<0) {

                Swal.fire('ขออภัย','แต้มสะสมของคุณมีน้อยเกินไป <br /> ไม่สามารถแก้ไขคำตอบได้ <br /> ต้องหาแต้มเพิ่มเติม','error' );
                ...
                resultResp = await axios.patch(`/users/${usePage().props.auth.user.id}/questions/${props.question.id}/answers/${props.question.authAnswerQuestion}`,
                { 
                    course_id: usePage().props.course.data.id,
                    answer_id: selectedOption.value,
                });
...
<template>
    <div class="w-full hs-accordion-group">
        <div class="hs-accordion active w-full" id="hs-basic-with-title-and-arrow-stretched-heading-one">
            <div class="flex items-center justify-between w-full overflow-visible">
                <button :id="`btn-hs-accordion-group${indexNumber}`"
                    class="bg-blue-200 p-6 rounded-md hs-accordion-toggle hs-accordion-active:text-blue-600 group py-3 inline-flex items-center justify-between gap-x-3 w-full font-medium text-left text-gray-800 transition hover:text-blue-900 dark:hs-accordion-active:text-blue-500 dark:text-gray-200 dark:hover:text-gray-400"
                    ...
                    <span>{{ props.question.points }} คะแนน</span>
                ...
                <div class="" v-if="props.isCourseOwner">
                    <VerticalDotsDropdownMenu
                        @delete-model="emit('delete-question')" 
                    >
            ...
            <div v-if="props.question.images" class="my-2">
                    ...
                    <div class="grid space-y-2">
                        <div v-for="(option, idx) in props.question.options" :key="option.id">
                            <div class="flex w-full justify-between items-center">
                                ...
                                <button :id="`btn-delete-option-${option.id}`" class="w-9 h-9 bg-red-200 flex items-center justify-center rounded-full" v-if="isCourseOwner" @click.prevent="deleteOption(option.id,idx)">
                                    <Icon icon="heroicons-outline:trash" class="w-5 h-5  text-red-600 font-bold"/>
                                </button>
                            </div>
