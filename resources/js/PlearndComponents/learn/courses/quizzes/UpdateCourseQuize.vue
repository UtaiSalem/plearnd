...
const start_date = ref(props.quiz.start_date);
const end_date = ref(props.quiz.end_date);
...
const form = reactive({
  ...
  shuffle_questions: props.quiz.shuffle_questions === 1 ? true : false,
  time_limit: props.quiz.time_limit,
  is_active: props.quiz.is_active === 1 ? true : false,
...
const handleUpdateQuizFormSubmit = async () => {
    ...
    try {
        ...
        const resp = await axios.put(`/courses/${props.quiz.course_id}/quizzes/${props.quiz.id}`, form);
