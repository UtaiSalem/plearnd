...
async function requestTobeCourseMember(){
    try {
        ...
        if (courseMemberResp && courseMemberResp.data.success) {
            // console.log(courseMemberResp.data);
...
const handleClink = () => {
    ...
    route('course.show', props.course.id);
}
...
<template>
    ...
    <button @click.prevent="handleClink" class=" bg-white rounded-xl shadow-lg overflow-hidden hover:scale-105">
        <div class="relative">
            <img class="w-full "
                :src="props.course.cover ? '/storage/images/courses/covers/'+props.course.cover : '/storage/images/courses/covers/default_cover.jpg'"
                alt="Colors" />
            <p class="absolute top-0 right-0 bg-yellow-300/75 text-gray-800 font-semibold py-1 px-2 rounded-bl-lg ">
            ...
            <p class="absolute bottom-0 right-0 bg-yellow-300/75 text-gray-800 font-semibold py-1 px-1 rounded-tl-lg">
                <span class="flex items-center justify-between text-sm text-white space-x-2">
        ...
        <Link :href="route('course.show', props.course.id)"  preserve-state class="bg-red-200">
            <h1 class="p-2 cursor-pointer hover:underline">
                <p class="text-gray-700 text-base font-bold ">
                    {{ props.course.name }} 
                </p>               
                <p v-if="props.course.code" class="text-gray-500 text-sm font-bold">{{ props.course.code }} </p>
                <p v-if="props.course.category" class="text-gray-500 text-sm font-bold">{{ props.course.category }} </p>
                <!-- <p class="text-gray-500 text-sm font-bold truncate">{{ props.course.description }} </p> -->
            </h1>
        </Link>

        <div class="flex justify-between  px-2">
            <div class="author flex items-center space-x-2">
                <div class="user-logo">
                    <img class="w-10 h-10 object-cover rounded-full shadow" 
                    :src="props.course.user.avatar"
                    alt="avatar">
                </div>
                <h2 class="flex flex-col">
                    <a href="#" class="text-md tracking-tighter text-gray-600">{{ props.course.user.name }}</a>
                    <span class="text-gray-400 text-xs">ผู้ดูแล</span> 
                </h2>
            </div>
        ...
        <div class=" p-2 flex flex-row flex-wrap justify-between">
        ...
        <!-- <div v-if="props.course.user.id !== $page.props.auth.user.id" class=" px-2 my-2">
            <button class="plearnd-btn-warning flex justify-center" :disabled="isLoading" v-if="props.course.member_status==3" @click.prevent="requestTobeUnmemberCourse">
            ...
            <button v-else-if="props.course.member_status==1" @click.prevent="requestTobeUnmemberCourse" :disabled="isLoading" class="plearnd-btn-secondary flex justify-center"
            ...
            <button v-else class="plearnd-btn-primary flex justify-center " :disabled="isLoading" @click.prevent="requestTobeCourseMember" v-if="!props.course.memberStatus">
