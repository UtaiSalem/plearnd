...
async function handleFormSubmit(){
    try {
            ...
            const cgResp = await axios.post(route('course.groups.store', props.course.id), newCourseGroup, config);
...
async function requestTobeUnMemberGroup(grpId, indx){
    ...
    try {
        ...
        if (unmemberGroupResp.data.success) {
            // if (memberResp.data.courseMember.status == 1){
                // usePage().props.course.data.isMember=true;
                // usePage().props.course.data.enrolled_students++;
                // usePage().props.groups.data[indx].members.push(memberResp.data.newCourseMember);
            // }
            
            usePage().props.courseMemberOfAuth          = unmemberGroupResp.data.courseMember;
            usePage().props.course.data.status          = memberResp.data.courseMember.status;
            usePage().props.course.data.member_status   = memberResp.data.courseMember.member_status;
            usePage().props.groups.data[indx].members   = memberResp.data.group.members;
            memberResp.data.courseMember.course_member_status == 1 ? usePage().props.course.data.isMember=true : usePage().props.course.data.isMember=false;
        ...
        <div class="flex flex-wrap  justify-between gap-4">
            <div v-for="(group, index) in computedGroup" :key="index" class="w-full sm:w-[48%]">
                <CourseGroupItem class=""
                    :group="group"
                    :courseMemberOfAuth="$page.props.courseMemberOfAuth"
                    :group_index="index"
