...
const newCommentForm = reactive({
    post_image_id: props.postImageID,
    content: ''
});

const canSubmit = computed( () => newCommentForm.content.length > 0 );

const handlePostImageCommentFormSubmit = async () => {
    try {
            isLoading.value = true;
            let newPostImageCommentResponse = await axios.post(`/courses/${props.courseId}/posts/${props.postId}/images/${props.postImageID}/comments`, newCommentForm);
            if(newPostImageCommentResponse && newPostImageCommentResponse.data.success){
                emit('add-new-comment', newPostImageCommentResponse.data.comment);
                // console.log(newPostImageCommentResponse.data.comment);
                newCommentForm.content = '';
                isLoading.value = false;
            }
            // console.log('Post comment form submit', newCommentForm.content, props.postImageID);
    } catch (error) {
        console.error(error);
    }
};
...
<template>
    <div class="">
        <form @submit.prevent="handlePostImageCommentFormSubmit" :id="`comment-form-${postImageID}`" class="mt-3 relative dark:border-secondary-800 flex items-center">
            ...
            <input type="text" v-model="newCommentForm.content" :id="`comment-form-comment-input-${postImageID}`" placeholder="แสดงความคิดเห็น..." class="text-sm ml-2 pl-3 pr-12 py-2 w-full dark:text-gray-600 rounded-lg focus:ring-0 border-gray-400">
            <div class="flex gap-4 absolute top-[7px] right-[60px]">
                <!-- <button @click.prevent="handleAddPostCommentImage" :disabled="isLoading">
            ...
            <button 
                ...
                :id="`comment-form-submit-button-${postImageID}`" 
