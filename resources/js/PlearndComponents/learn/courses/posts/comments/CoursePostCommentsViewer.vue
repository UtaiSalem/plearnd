...
const showCommentsModalHandler = async () => {
    try {
        ...
        // let commentsResponse = await axios.get(`/${props.post.post_url}/comments`);
...
<template>
    <div>
        <div>
            ...
            <CoursePostCommentItem v-for="(comment, index) in initComments" :key="comment.id" :comment="comment" :isPostOwner @delete-post-comment="handleDeletePostComment(index)" />
