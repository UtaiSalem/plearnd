<script setup>
    ...
    import IndividualPostImagesViewer from '@/PlearndComponents/learn/courses/posts/PostViewerPartials/IndividualCoursePostImageViewer.vue';
    ...
    const selectedPrivacy = ref(props.activity.target_resource.privacy_settings);
    ...
    const canBePosted = computed(() => postForm.value.content.length>0 || postForm.value.images.length>0);
    ...
    const postForm = ref({
        content: props.activity.target_resource.content,
        point:1,
        privacy_settings: props.activity.target_resource.privacy_settings,
        images: tempPostImages,
    });

    async function onSubmitUpdatePost(){
        ...
        if(!canBePosted.value ) {
            ...
            return;
        }else if(postForm.value.content.length>5000){
            Swal.fire({
                icon: 'error',
            ...
            return;
        }else if(postForm.value.images.length>20){
            Swal.fire({
                icon: 'error',
            ...
            return;
        }else if(authUser.pp < 36){
            Swal.fire({
                icon: 'error',
                title: 'แต้มใช้งานสะสมไม่เพียงพอ',
                text: 'คุณต้องมีแต้มใช้งานสะสมมากกว่า 360 คะแนน กรุณาสะสมแต้มเพิ่มเติม',
            });
            return;
        };
        ...
        formDataUpdate.append('content', postForm.value.content);
        formDataUpdate.append('privacy_settings', selectedPrivacy.value);
        formDataUpdate.append('point', postForm.value.point);
        Array.from(tempPostImages).forEach(image => {
            formDataUpdate.append('images[]', image.file);
        });

        formDataUpdate.append('_method', 'patch');
        // const postResponse = await axios.post(`/posts/${props.activity.target_resource.id}`, formDataUpdate, config);
        const postResponse = await axios.post(props.activity.target_resource.post_url, formDataUpdate, config);
        ...
        if(postResponse.data.success){
            ...
            Swal.fire({
                ...
                text: 'โพสต์ของคุณได้รับการแก้ไขเรียบร้อยแล้ว',
            });
            // router.go(-1);
            // window.location.href = `/posts/${props.activity.target_resource.id}`;
            props.activity.target_resource = postResponse.data.post;
            // postForm.images = response.data.post_images;
            // console.log(postResponse.data.post);
            // props.activity.target_resource.images = postResponse.data.post.images;
            tempPostImages.splice(0);
        } else{
            Swal.fire({
                icon: 'error',
                title: 'แก้ไขโพสต์ไม่สำเร็จ',
                text: 'โพสต์ของคุณไม่สามารถแก้ไขได้ในขณะนี้ กรุณาลองใหม่อีกครั้ง',
            });
        }

    }

    function handleCancleEditPost(){
        // postForm.reset();
        postForm.content = props.activity.target_resource.content;
        tempPostImages.splice(0);
    ...
    <div class="relative px-4 py-2 bg-white rounded-lg shadow-lg hs-accordion-group">
        <div class="hs-accordion active" id="hs-basic-with-title-and-arrow-stretched-heading-one">
            ...
            <div id="hs-basic-with-title-and-arrow-stretched-collapse-one" class="hs-accordion-content w-full overflow-hidden transition-[height] duration-300" aria-labelledby="hs-basic-with-title-and-arrow-stretched-heading-one">
                <div class="px-1 py-2">
                    <form @submit.prevent="onSubmitUpdatePost" id="UpdatePostForm">
                        <div class="border-b border-secondary-200 dark:border-secondary-800">
                            <Textarea 
                                ...
                                rows="2"
                                autoResize
                                class="block p-2.5 mb-2 w-full min-h-fit text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            >
                            </Textarea>
                        </div>
                        ...
                        <div class="flex items-center justify-between mx-1 mt-4">
                            ...
                            <div class="flex space-x-2">
                                <button type="button" @click.prevent="handleCancleEditPost" class="px-2 py-1 text-sm text-red-500 bg-gray-100 border border-red-500 rounded-lg dark:text-white hover:scale-110">ยกเลิก</button>
                                <button type="submit" :disabled="waiting || !canBePosted" class="px-2 py-1 text-sm text-blue-500 bg-blue-100 border border-blue-500 rounded-lg dark:text-white disabled:cursor-not-allowed" :class="canBePosted ? 'hover:scale-110' : 'cursor-not-allowed'">บันทึก</button>
                            </div>
                        </div>
                    </form>
                </div>
                
                <IndividualPostImagesViewer v-if="activity.target_resource.images.length>0"
