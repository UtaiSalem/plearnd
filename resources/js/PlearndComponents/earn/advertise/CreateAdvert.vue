...
const timeToShowProductMediaOptions = ref(generateOptions(5, 30, 5));
...
function computeQuantityToShowProductMedia(mIdx=0,tIdx=0){
    ...
    quantityToShowProductMedia.value = Math.floor(((totalMoneySupport.value)*1080)/(timeToShowProductMedia.value*54));
    bonus.value = totalMoneySupport.value >= 50 && timeToShowProductMedia.value >= 10 ? quantityToShowProductMedia.value + timeToShowProductMedia.value : quantityToShowProductMedia.value;
}
...
async function submitForm(e) {
  ...
  supportData.append('supporter', props.Supporter.data.id);
  ...
  supportData.append('transfer_date', transferDate.value.toDateString());
  ...
  try {
    const supportResp = await axios.post('/supports/advertise', supportData, config);
...
<template>
    <div class="plearnd-card mt-4">
        ...
        <div
            class="max-w-sm space-y-2 sm:flex sm:items-center sm:space-y-0 sm:space-x-2"
        >
            <img
        ...
        >
            <div class="grid grid-cols-1 gap-4 mt-4">
                <div>
                    ...
                    <select
                        ...
                        @change.prevent="
                            computeQuantityToShowProductMedia(
                                moneyIndexSelected,
                                timeIndexSelected
                            )
                        "
            ...
            <div class="grid grid-cols-1 gap-4 mt-4">
                <div>
                    <label
                        for="time-to-show-Product-media-dropdown"
                        class="block text-gray-700 dark:text-white mb-1 w-3/4"
                        >ระยะเวลาแสดงผลสื่อประชาสัมพันธ์สินค้า(วินาที/ครั้ง)</label
                    >
                    <select
                        id="time-to-show-Product-media-dropdown"
                        v-model="timeIndexSelected"
                        @change.prevent="
                            computeQuantityToShowProductMedia(
                                0,
                                timeIndexSelected
                    ...
                    >
                        ...
                        <option
                            v-for="(
                                timeOption, tidx
                            ) in timeToShowProductMediaOptions"
                            :key="tidx"
                            :value="tidx"
                            :selected="timeOption.value == 1"
                        >
                            {{ timeOption }}
                        </option>
                    </select>
            ...
            <div class="grid grid-cols-1 gap-4 mt-4">
                <div>
                    <p class="block text-gray-700 dark:text-white mb-1">
                        จำนวนที่จะแสดงผลสื่อประชาสัมพันธ์สินค้า
                    </p>
                    <p
                        class="w-full rounded-lg border border-gray-400 bg-gray-300 text-center py-2 px-3 dark:bg-gray-700 dark:text-white dark:border-none"
                    >
                    ...
                    <div class="text-center">
                        ...
                        <h3 class="mt-2 text-sm font-medium text-gray-900">
                            <p class="relative">
                                ...
                                <button
                                    class="plearnd-btn-primary w-36 mx-2"
                                    @click.prevent="browseInputSlip"
                                >
                                    or browse
                                </button>
                                <span>to upload</span>
                            ...
                            <input
                                ...
                                ref="inputSlip"
                                class="hidden"
                                @change.prevent="onInputSlipChange"
                            />
                        </h3>
            ...
            <div class="grid grid-cols-1 gap-4 mt-2">
                <div v-if="mediaImage">
                    <div class="relative mb-2 max-h-fit overflow-hidden">
                        ...
                        <button
                            type="button"
                            @click.prevent="deleteMediaImage"
                            class="absolute top-1 left-1 rounded-full cursor-pointer bg-gray-100 p-2"
                        >
                            <Icon
                                icon="fa-solid:trash-alt"
                    ...
                    <div class="text-center">
                        ...
                        <h3 class="mt-2 text-sm font-medium text-gray-900">
                            <p class="relative">
                                ...
                                <button
                                    class="plearnd-btn-primary w-36 mx-2"
                                    @click.prevent="browseInputMedia"
                                >
                                    or browse
                                </button>
                                <span>to upload</span>
                            ...
                            <input
                                ...
                                ref="inputMediaImage"
                                class="hidden"
                                @change.prevent="onInputMediaChange"
                            />
                        </h3>
                        <p class="mt-1 text-xs text-gray-500">
                            PNG, JPG, GIF up to 10MB
